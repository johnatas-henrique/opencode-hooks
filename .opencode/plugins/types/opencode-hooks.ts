...
export type OpenCodeEventType = (typeof OpenCodeEvents)[keyof typeof OpenCodeEvents]
...
export type OpenCodeEventMap = {
  ...
  [OpenCodeEvents.FILE_WATCHER_UPDATED]: Event & { type: "file.watcher.updated" }
  [OpenCodeEvents.VCS_BRANCH_UPDATED]: Event & { type: "vcs.branch.updated" }
  [OpenCodeEvents.INSTALLATION_UPDATED]: Event & { type: "installation.updated" }
  [OpenCodeEvents.INSTALLATION_UPDATE_AVAILABLE]: Event & { type: "installation.update-available" }
  [OpenCodeEvents.LSP_CLIENT_DIAGNOSTICS]: Event & { type: "lsp.client.diagnostics" }
  [OpenCodeEvents.LSP_UPDATED]: Event & { type: "lsp.updated" }
  [OpenCodeEvents.MESSAGE_PART_REMOVED]: Event & { type: "message.part.removed" }
  [OpenCodeEvents.MESSAGE_PART_UPDATED]: Event & { type: "message.part.updated" }
  ...
  [OpenCodeEvents.SERVER_INSTANCE_DISPOSED]: Event & { type: "server.instance.disposed" }
...
export type OpenCodeEventProperties<T extends OpenCodeEventType> = OpenCodeEventMap[T]["properties"]

export type OpenCodeEventHandler<T extends OpenCodeEventType> = (
  input: { event: OpenCodeEventMap[T] }
) => Promise<void>
