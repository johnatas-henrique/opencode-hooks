...
## Nova Abordagem: Híbrido B + C para Archive

**Data**: 2026-04-21 (atualizado)

### Decisão sobre Arquivamento

| Aspecto             | Antes              | Agora (Híbrido B + C)     |
| ------------------- | ------------------ | ------------------------- |
| **Quando archiva**  | No startup + lock  | Se ≥ 1MB OU shutdown      |
| **Lock necessário** | ✅ Sim             | ❌ Não (sem race)         |
| **Async**           | Sim                | Sync (rename é O(1))      |
| **Complexidade**    | Alta               | ✅ Baixa                  |
| **Perda de dados**  | Múltiplos arquivos | Máximo 1MB (último chunk) |

### Por que não precisamos de lock?

- `fs.rename` é O(1) - **INSTANTÂNEO**, não lê conteúdo
- Não archiva no startup - **Sem race condition**
- Cada processo archiva só no seu shutdown
- Arquivo por tamanho (≥ 1MB) é independente
...
Arquivo cresce...
  ↓
Arquivo ≥ 1MB → archiva (fs.rename instantâneo)
  ↓
Nova sessão continua com arquivo vazio
  ↓
Sessão fecha (shutdown) → archiva resto
```

### Próximos Passos (Passo 6 Atualizado)

| Passo | Ação                                  | Prioridade |
| ----- | ------------------------------------- | ---------- |
| 6a    | Remover archive do startup            | Alta       |
| 6b    | Implementar archive por tamanho (1MB) | Alta       |
| 6c    | Adicionar hook no shutdown            | Alta       |
| 6d    | Remover lock (não precisa mais)       | Alta       |
...
### Passo 6: Lógica Híbrida de Arquivamento (Problema 7)
...
**Solução Nova**: Abordagem híbrida (B + C) - sem lock, mais simples:

#### Por que não precisamos de lock?

| Momento        | Processo A       | Processo B  | Precisa lock? |
| -------------- | ---------------- | ----------- | ------------- |
| Startup        | Não archiva      | Não archiva | ❌ Não        |
| Durante sessão | Archiva se ≥ 1MB | -           | ❌ Não        |
| Shutdown       | Archiva resto    | -           | ❌ Não        |
...
// 1. Archive quando arquivo ≥ 1MB (após cada write)
async function writeLine(filePath: string, data: any): Promise<void> {
...
// 2. Archive no shutdown (via server.instance.disposed)
export function initArchiveOnShutdown(files: string[]): void {
...
// 3. Função simples de archive (fs.rename é O(1), não precisa lock
async function archiveFile(sourcePath: string): Promise<void> {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  const dir = path.dirname(sourcePath);
  const ext = path.extname(sourcePath);
  const base = path.basename(sourcePath, ext);
  const targetDir = path.join(dir, 'audit-archive');

  // Criar arquivo com timestamp
  const targetPath = path.join(targetDir, `${base}-${timestamp}${ext}`);

  // Mover (não copiar - é O(1))
...
- Arquivo cresce até 1MB → archiva automaticamente
- Sessão fecha → archiva o que restou
- Próxima sessão → começa com arquivo novo vazio
- Sem race condition (não archiva no startup)
- Sem lock (rename é instantâneo O(1))
